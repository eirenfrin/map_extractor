...
To delete all files and folders associated with a specific map run:

```
python main.py delete-map-files --name <YOUR_MAP_TITLE>
```

File `/boundaries/<YOUR_MAP_TITLE>.json.`, folder `/tiles/<YOUR_MAP_TITLE>` and its contents as well as`<YOUR_MAP_TITLE>.png` image will be deleted.
...
The tool enforces the same map title for all artifacts (`.json` file, `.png` of the entire map, folder titles) related to the same map. If a folder or a file with the given name already exists, you can either delete/relocate it or choose a different map title.
...
**1.0.0**
Intial version with the basic functionalities of collection of boundaries, taking screenshots and stitching them into a complete map fragment.

**1.1.0**
Current version includes map image size optimization and automatic deletion of all map artifacts using a single command.
